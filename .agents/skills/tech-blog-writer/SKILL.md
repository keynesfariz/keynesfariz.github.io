...
2. Once provided, convert each image to WebP using `cwebp`.
   ...
   ```
   _Example: If the post slug is `spendernote` and the image is a transaction page screenshot, use `public/assets/spendernote-transaction-page.webp`._
3. Generate the responsive breakpoints by running the processing script on the newly created webp image:
   ```bash
   bun scripts/process-image.ts public/assets/<main-post-slug>-<image-context>.webp
   ```
